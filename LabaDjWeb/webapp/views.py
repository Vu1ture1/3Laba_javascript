...
from .models import MyUser, Medicine, PharmacyDepartment, Order, MedSupplier, BasketItem, PickUpPoint, Faq, News, MonthPlot,Review, Employee, Vacancy, AboutCompany, MostAskedQuestions
...
def index(request):
    com = AboutCompany.objects.first()

    sel = request.GET.get('type_of_medicine')

    all_deps = PharmacyDepartment.objects.all()

    deps = PharmacyDepartment.objects.all()

    if sel != None:
        deps = PharmacyDepartment.objects.filter(type_of_medicine=sel)
    ...
    return TemplateResponse(request, "index.html", context={"all_deps": all_deps, "deps": deps, "news": news, "company": com, "selected_type": sel, "user_ord": len(myorders) if myorders else 0})
...
def faq(request):
    ...
    return TemplateResponse(request, "faq.html", {"info": faq, "vacs": vacs, "company": com})
...
def graph_js(request):
    return TemplateResponse(request, "graph_js.html")
...
def certificate(request):
    ...
    return TemplateResponse(request, "certificate.html", context={"company": com})
...
def article(request):
    ...
    return TemplateResponse(request, "article.html", context={"article": news})

def etc(request):
    return TemplateResponse(request, "etc.html")

def questions(request):
    all_questions = MostAskedQuestions.objects.all()
    
    return TemplateResponse(request, "questions.html", context={"all_questions": all_questions})