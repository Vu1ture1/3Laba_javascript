...
class News(models.Model):
    ...
    news_message = models.CharField(blank=False, default="Содержание", max_length=10000)
    date_created = models.DateTimeField(auto_now_add=True)
...
class AboutCompany(models.Model):
    ...
    video = models.FileField(blank=False, upload_to="media/videos/")
    
    logo = models.ImageField(blank=False, upload_to="media/images/")
    
    certificate = models.ImageField(blank=True, upload_to="media/images/")
    
    year_history = models.CharField(blank=False, default=None, max_length=10000)

    # реквизиты

    # КПП (Код причины постановки на учет) 

    RCR = models.CharField(blank=False, default=None, max_length=125)

    # ИНН (Идентификационный номер налогоплательщика)

    TIN = models.CharField(blank=False, default=None, max_length=125)

    # ОГРН (Основной государственный регистрационный номер)

    PSRN = models.CharField(blank=False, default=None, max_length=125)

    bank_details = models.CharField(blank=False, default=None, max_length=125)

class MostAskedQuestions(models.Model):

    name = models.CharField(blank=False, default="Содержание", max_length=200)
    
    description = models.CharField(blank=False, default="Содержание", max_length=2000)